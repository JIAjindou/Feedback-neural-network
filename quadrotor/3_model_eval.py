...
params_NODE = ca.DM(np.load("learning_NODE/temp/model_param.npy"))      # DM is used for end2end forward rollout
params_MLP = np.load("learning_MLP/temp/model_param.npy")               # or use ca.DM(*).full()
...
def rollout_and_pred(refk_seq, x0, params):
    ...
    xk_real_seq = [xk_real]
    xk_pred_seq = [xk_pred]
    drag_real_seq = [model_sim.aerodrag_sym(xk_real).full()]
    drag_pred_seq = [model_learn.NN_model(xk_real[3:9], params).full()]
    ...
    for i in range(refk_seq.shape[0]):
        ...
        drag_real = model_sim.aerodrag_sym(xk1_real).full()
        drag_pred = model_learn.NN_model(xk1_real[3:9], params).full()
        xk_real_seq.append(xk1_real.full())
        xk_pred_seq.append(xk1_pred.full())
        drag_real_seq.append(drag_real)
        drag_pred_seq.append(drag_pred)
    ...
    drag_real_seq = np.array(drag_real_seq).reshape(-1, drag_real.shape[0])
    drag_pred_seq = np.array(drag_pred_seq).reshape(-1, drag_pred.shape[0])
...
for i, traj_dir in enumerate(traj_dir_list):
    ...
    (
        ...
        drag_real_seq,
        drag_pred_seq,
    ) = rollout_and_pred(refk_seq, x0, params=params_MLP)

    savemat(
        "train_eval/train_trajectory{}_MLP.mat".format(traj_i),
        {
            "xk_real_seq": xk_real_seq.T,
            "xk_pred_seq": xk_pred_seq.T,
            "refk_seq": refk_seq.T,
            "drag_real_seq": drag_real_seq.T,
            "drag_pred_seq": drag_pred_seq.T,
        },
    )
    ...
    (
        ...
        drag_real_seq,
        drag_pred_seq,
    ) = rollout_and_pred(refk_seq, x0, params=params_NODE)
...
while traj_i <= traj_num:
    ...
    if max(vel_ref) <= 10:
        ...
        savemat(
            "test_eval/test_trajectory{}_MLP.mat".format(traj_i),
            {
                "xk_real_seq": xk_real_seq.T,
                "xk_pred_seq": xk_pred_seq.T,
                "refk_seq": refk_seq.T,
