...
true_A_norminal = torch.tensor([[-0.14, 3.2], [-3.2, -0.14]]).to(device)  # Parameter of training function
...
class Lambda_norminal(nn.Module):
    def forward(self, t, y):
        ...
        return torch.mm(y, true_A_norminal) + 8.
...
def visualize(dydt_true_norminal, dydt_ODEnn_test, dydt_FNN_test, Total_loss_sequence):
    if args.viz:
        makedirs('png')

        fig = plt.figure(figsize=(12, 4), facecolor='white')

        ax_dydt_ODEnn_test = fig.add_subplot(131, frameon=True)
        ax_dydt_FNN_test = fig.add_subplot(132, frameon=True)
        ...
        ax_dydt_ODEnn_test.set_title('Test performance of Neural ODE', fontsize=15, pad=10)
        ax_dydt_ODEnn_test.set_xlabel('$\dot{x}$', fontsize=15)
        ax_dydt_ODEnn_test.set_ylabel('$\dot{y}$', fontsize=15)
        ax_dydt_ODEnn_test.tick_params(axis='x', labelsize=14)
        ax_dydt_ODEnn_test.tick_params(axis='y', labelsize=14)

        x_true = dydt_true_norminal.detach().numpy()[:, 0]
        y_true = dydt_true_norminal.detach().numpy()[:, 1]
        x_test = dydt_ODEnn_test.cpu().detach().numpy()[:, 0, 0]
        y_test = dydt_ODEnn_test.cpu().detach().numpy()[:, 0, 1]
        Error_xy = np.sqrt((x_test - x_true) ** 2 + (y_test - y_true) ** 2)
        upper = Error_xy.max()

        ax_dydt_ODEnn_test.plot(x_true, y_true, '--', color=color_gray, linewidth=1.5, label='Truth')
        ax_dydt_ODEnn_test.scatter(x=x_true[0], y=y_true[0], s=100, marker='*', color=color_gray)
        ax_dydt_ODEnn_test.scatter(x=x_test[0], y=y_test[0], s=100, marker='*', color=color_red)

        points = np.array([x_test, y_test]).T.reshape(-1, 1, 2)
        segments = np.concatenate([points[:-1], points[1:]], axis=1)
        norm = plt.Normalize(0, upper)
        ...
        ax_dydt_ODEnn_test.legend(loc='lower right', fontsize=13)
        ax_dydt_ODEnn_test.set_aspect('equal', adjustable='box')

        # Figure-2: Feedback neural networks
        ax_dydt_FNN_test.cla()
        ax_dydt_FNN_test.set_title('Test performance of feedback neural network', fontsize=15, pad=10)
        ...
        x_true = dydt_true_norminal.detach().numpy()[:args.data_size - 1, 0]
        y_true = dydt_true_norminal.detach().numpy()[:args.data_size - 1, 1]
...
if __name__ == '__main__':
    # load trained_model
    funcODE = torch.load('../spiral_model.pt', weights_only=False).to(device)
    ...
    # optimizer = optim.Adam(funcBack.parameters(), lr=1e-2, betas=(0.9, 0.99))
    ...
    label_true = (true_y_domran[:, 1:, :, :] - true_y_domran[:, :args.data_size - 1, :, :]
                  - sample_time * dydt_ODEnn[:, :args.data_size - 1, :,
                                  :]) / sample_time  # [case_num, data_size - 1, 1, 2]
    ...
    #     end = time.time()
    # torch.save(funcBack, 'D:/Code_workspace/Neural_ODE_my/project0807/spiral_model_Feedback_part2.pt')

    funcBack = torch.load('../trained_model/spiral_model_Feedback_part2.pt', weights_only=False).to(device)
    ...
    dydt_ODEnn_test = funcODE(0, true_y_norminal).to(device)
    ...
    for j in range(args.data_size - 1):
        y0_test = true_y_norminal[j, :, :]  # Initial value at each moment
    ...
    visualize(dydt_true_norminal, dydt_ODEnn_test, dydt_FNN_test, Total_loss_sequence)